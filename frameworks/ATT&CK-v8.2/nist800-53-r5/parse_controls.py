...
class Control:
    ...
    def format_description(self):
        ...
        descr = []
        if self.text:
            descr.append(self.text)
        if self.discussion:
            descr.append(self.discussion)
        return "\n\n".join(descr)
...
def parse_controls(controlpath, control_ids, relationship_ids, config_location):
    ...
    :param control_ids: is a dict of format {control_name: stixID}
        which maps control names (e.g AC-1) to desired STIX IDs
