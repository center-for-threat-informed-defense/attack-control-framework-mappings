...
def row_type(row):
    """from a pandas series determine if it's a "control", "control_enhancement" or
    "statement" and return the type as a string"""
...
class Control:
    ...
    def __init__(self, row, control_ids, parent=None):
        ...
        self.stix_id = (control_ids[self.external_id]
                        ...
                        else f"course-of-action--{str(uuid.uuid4())}")
...
def parse_controls(controlpath, control_ids, relationship_ids, config_location):
    """parse the NIST800-53 revision 4 controls and return a STIX bundle
    :param controlpath: the filepath to the controls TSV file
    :param control_ids is: a dict of format {control_name: stixID} which
        maps control names (e.g AC-1) to desired STIX IDs
    ...
    for control in tqdm(list(filter(lambda c: control.parent_id or len(control.related) > 0, controls)),
                        desc="creating control relationships", bar_format=tqdmformat):
    ...
    return Bundle(*itertools.chain(stixcontrols, relationships), allow_custom=True)
